...
-------------
 ...
 ------------
  ...
  RHCE - Network Administration

 System Administration
----------------------
 => Working with Linux Desktop
 => linux command line utilities
 => Linux Text processing tools (cat,tail,grep,head,cut,pipe,less,more)
 => Linux Text Editor
 => user and group addministration
 => permission and ownership
 => disk managagement (lvm,iscsi,swap)
 => process monitoring
 => job scheduling
...
------------------------------- 
 => SSH application service
 => NFS server & Automount 
 => proxy server with Monitoring 
 => DNS server (Primary DNS)
 => Mail server (postfix+squirrelmail+webmail+outlook)
 => Linux security with firewall & Iptables
 => Monitoring (cacti)
 => Samba file server 
 => iscsi server (storage Server)
 => database with MariadB
 => Apache HTTP Webserver (http and https and virutal hosting)
 => shell scripting
